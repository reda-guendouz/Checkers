...
void affiche_piece(INTERFACE_GRAPHIQUE ig,PIECE P, POINT p, THEME th) {
	...
	if (ig == CLASSIQUE) {
		...
		if (P.coulP == NOIR) 
			draw_fill_circle(p,RAYON,th.pionSombre);
		else 
			draw_fill_circle(p,RAYON,th.pionClair);
		p1.x -= 5; p1.y += 17;
		if (P.typeP == DAME)
			aff_pol("D",22,p1,noir);
	}
	else {
		p1.x = p.x - 17; p1.y = p.y;
		p2.x = p.x;	     p2.y = p.y - 17;
		p3.x = p.x + 17; p3.y = p.y;
		if (P.coulP == NOIR) {
			draw_fill_triangle(p1,p2,p3,th.pionSombre);
			draw_triangle(p1,p2,p3,noir);
			p2.y = p.y + 17;
			draw_fill_triangle(p1,p2,p3,th.pionSombre);	
		}
		else {
			draw_fill_triangle(p1,p2,p3,th.pionClair);
			draw_triangle(p1,p2,p3,noir);
			p2.y = p.y + 17;
			draw_fill_triangle(p1,p2,p3,th.pionClair);
		}
...
void efface_piece(INTERFACE_GRAPHIQUE ig, POINT p1, THEME th) {
	if (ig == CLASSIQUE) {
...
void affiche_themes(INTERFACE_GRAPHIQUE ig) {	
	...
	for (i = 0; i!= 4;i++) {
		...
		if (i == 2) {
			p1.x = 325 + 300; p1.y = 400  - 200;
		}
		else if (i == 1) {
			p1.y = 400 - 200; p1.x = 325; 
		}
		else if (i == 0)
			p1.x = 325 + 300;
	}
...
void affiche_damier(INTERFACE_GRAPHIQUE ig, THEME th) {
	...
	for (i = 0; i<10; i++) {
		for (j = 0; j<10; j++) {
			...
			if ((i+j)%2 == 0)
				affiche_case(ig,p1, th.caseClaire);
			else { 
				affiche_case(ig,p1,th.caseSombre);
				if (ig == CLASSIQUE) {
					p1.x += TAILLE_CASE/2;
...
void affiche_menu_fleche_theme(THEME th, BOOL afficheEfface) {
	POINT p1, p2, p3;
	COULEUR clr = noir, thActuel = th.caseSombre;
...
void affiche_joueur(COULEUR joueurActuel,INTERFACE_GRAPHIQUE ig, THEME th) {
	...
	P.typeP = PION;	P.coulP = joueurActuel;
	p1.x = 575; p1.y = 550;
	p2.x = 625; p2.y = 600;
...
void affiche_plateau(INTERFACE_GRAPHIQUE ig, THEME th) {	
	...
	affiche_damier(ig,th);
}


void affiche_efface_cases_possibles(POINT* pointsCasesPossibles, int nombreCasesPossibles, INTERFACE_GRAPHIQUE ig,THEME th,BOOL affiche) {
	POINT p1;
	int i;
	...
	for (i = 0; i <nombreCasesPossibles; i++) {
		...
		if (affiche)
			affiche_case(ig,p1,rouge); // affiche surbrillance
		else 
			affiche_case(ig,p1,th.caseSombre); // efface surbrillance
	}
...
void affiche_pieces_prises(COULEUR clr, INTERFACE_GRAPHIQUE ig, THEME th, int nbPiecesPrises) {
	...
	P.typeP = PION;
	if (clr == BLANC) {
		p1.x = 50 + (TAILLE_CASE*(nbPiecesPrises/6)); 
		p1.y =  400 - (TAILLE_CASE * (nbPiecesPrises%6));
		P.coulP = NOIR;
	}
	else {
		p1.x = 1000 - (TAILLE_CASE*(nbPiecesPrises/6));
		p1.y = 400 -(TAILLE_CASE * (nbPiecesPrises%6));
		P.coulP = BLANC;
	}
	affiche_piece(ig,P,p1,th);
